...
shared_ptr<uint16_t> rpn_calc(command const cmd, uint16_t const value = 0) {
    ...
    if (cmd == cmd_left_shift) {
        ...
        uint16_t shift = rpn_stack.back();
         rpn_stack.pop_back();

        uint16_t number = rpn_stack.back();
        rpn_stack.pop_back();
    ...
    if (cmd == cmd_add) {
        ...
        uint16_t b = rpn_stack.back();
        uint16_t a = rpn_stack[rpn_stack.size() - 2];
